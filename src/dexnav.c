...
static void DrawDexNavSearchMonIcon(u16 species, u8 *dst, bool8 owned)
{
    ...
    spriteId = CreateMonIcon(species, SpriteCB_MonIcon, SPECIES_ICON_X - 6, GetSearchWindowY() + 8, 0, 0); // , 0xFFFFFFFF
...
static void TryDrawIconInSlot(u16 species, s16 x, s16 y)
{
    if (species == SPECIES_NONE)
        ...
        CreateMonIcon(SPECIES_NONE, SpriteCB_MonIcon, x, y, 0, 0); //question mark , 0xFFFFFFFF
    else
        CreateMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0); // , 0xFFFFFFFF
}
...
static void DrawSpeciesIcons(void)
{
    ...
    for (i = 0; i < HIDDEN_WILD_COUNT; i++)
    {
        ...
        if (FlagGet(FLAG_SYS_DETECTOR_MODE))
            ...
            CreateMonIcon(SPECIES_NONE, SpriteCB_MonIcon, x, y, 0, 0); //question mark if detector mode inactive , 0xFFFFFFFF
    }
}
